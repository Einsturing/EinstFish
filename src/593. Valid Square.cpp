...
public:
    ...
    bool calCos(vector<int> &v1, vector<int> &v2)
    {
        return (v1[0] * v2[0] + v1[1] * v2[1]) == 0;
    }
    bool fun(vector<int> &p1, vector<int> &p2, vector<int> &p3, vector<int> &p4)
    {
        vector<int> v1 = {p1[0] - p2[0], p1[1] - p2[1]};
        vector<int> v2 = {p3[0] - p4[0], p3[1] - p4[1]};
        if (checkLength(v1, v2) && checkMidPoint(p1, p2, p3, p4) && calCos(v1, v2))
            return true;
        return false;
    }
